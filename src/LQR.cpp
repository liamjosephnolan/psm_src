...
float compute_LQR_control(float* gains, float commanded_position, float actual_position) {
    ...
    static unsigned long previous_time = 0; // Store the previous time
    ...
    if (delta_time <= 0.0f) delta_time = 0.001f; // Prevent division by zero

    // Calculate velocity
    velocity = (actual_position - previous_position) / delta_time;
    previous_position = actual_position; // Update the previous position
